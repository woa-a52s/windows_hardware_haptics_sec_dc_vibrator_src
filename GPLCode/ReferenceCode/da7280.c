...
struct haptic_cmd_effect {
	UINT16 effect_type;
	UINT16 replay_length;
	INT16 constant_level;
	/* periodic waveform */
	UINT16 p_waveform;
	/* Periodic custom data */
	INT16 p_cust_data[DA7280_PARAM_LEN];
	/* periodic magnitude */
	INT16 p_magntd;
};
...
struct da7280_haptic {
	...
	UINT32 imax;
	UINT32 impd;
	UINT32 resonant_freq_h;
	UINT32 resonant_freq_l;
	BOOLEAN bemf_sense_en;
	BOOLEAN freq_track_en;
	BOOLEAN acc_en;
	BOOLEAN rapid_stop_en;
	BOOLEAN amp_pid_en;
	UCHAR ps_seq_id;
	UCHAR ps_seq_loop;
	struct da7280_gpi_ctl gpi_ctl[3];
	BOOLEAN mem_update;
	UCHAR snp_mem[DA7280_SNP_MEM_SIZE];
	UCHAR enabled;
	int length_us;
	...
	struct mutex mutex;
	/* seq_id_duration */
	UINT32 seq_id_dur[DA7280_SEQ_ID_MAX + 1];
...
static BOOLEAN da7280_volatile_register(struct device* dev, unsigned int reg)
{
	switch (reg) {
...
static int da7280_haptic_set_pwm(struct da7280_haptic* haptics)
{
	...
	UINT64 period_mag_multi;
	unsigned int pwm_duty;
	int error;

	pwm_get_args(haptics->pwm_dev, &pargs);
	period_mag_multi =
		(UINT64)(pargs.period * haptics->magnitude);
	if (haptics->acc_en)
		pwm_duty =
	...
	int gain)
{
	...
	case DA7280_DRO_MODE:
		if (haptics->acc_en == 1)
			haptics->level = (UCHAR)(gain * 0x7F / 0x7fff);
		else if (haptics->acc_en == 0) {
			haptics->level = (UCHAR)(gain * 0xFF / 0x7fff);
...
static void da7280_haptics_set_gain(struct input_dev* dev, UINT16 gain)
{
	struct da7280_haptic* haptics = input_get_drvdata(dev);
	unsigned long flags;
	struct haptic_cmd_data cmd_data;
...
static UCHAR da7280_haptic_of_mode_str(struct device* dev,
	const char* str)
{
	if (!strcmp(str, "LRA"))
		return DA7280_LRA;
...
static UCHAR da7280_haptic_of_gpi_mode_str(struct device* dev,
	const char* str)
{
...
static UCHAR da7280_haptic_of_gpi_pol_str(struct device* dev,
	const char* str)
{
...
static UCHAR da7280_haptic_of_volt_rating_set(UINT32 val)
{
	UINT32 voltage;
	...
	struct da7280_haptic* haptics)
{
	...
	if (device_property_read_UINT32(dev, "dlg,op-mode", &val) >= 0)
	...
	if (device_property_read_UINT32(dev, "dlg,nom-microvolt", &val) >= 0)
	...
	if (device_property_read_UINT32(dev, "dlg,abs-max-microvolt",
		&val) >= 0)
		if (val < DA7280_VOLTAGE_RATE_MAX)
			haptics->absmax =
			da7280_haptic_of_volt_rating_set(val);
	...
	if (device_property_read_UINT32(dev, "dlg,imax-microamp", &val) >= 0)
	...
	if (device_property_read_UINT32(dev, "dlg,impd-micro-ohms", &val) >= 0)
	...
	if (device_property_read_UINT32(dev, "dlg,resonant-freq-hz",
		&val) >= 0) {
	...
	if (device_property_read_UINT32(dev, "dlg,ps-seq-id", &val) >= 0)
	...
	if (device_property_read_UINT32(dev, "dlg,ps-seq-loop", &val) >= 0)
	...
	for (i = 0; i < 3; i++) {
		gpi_str1[7] = '0' + i;
		if (device_property_read_UINT32(dev, gpi_str1, &val) >= 0)
			if (val <= DA7280_SEQ_ID_MAX)
	...
	for (i = 0; i <= DA7280_SEQ_ID_MAX; i++) {
		...
		if (device_property_read_UINT32(dev, duration_us, &val) >= 0) {
	...
	if (device_property_read_UINT32_array(dev, "dlg,mem-array",
		&mem[0],
 ...
 * 0 <= X <= 15.
 */
static SIZE_T ps_seq_id_store(struct device* dev,
...
static SIZE_T ps_seq_id_show(struct device* dev,
	struct device_attribute* attr,
	char* buf)
{
	struct da7280_haptic* haptics = dev_get_drvdata(dev);
 ...
 * 0 <= X <= 15.
 */
static SIZE_T ps_seq_loop_store(struct device* dev,
	struct device_attribute* attr,
	const char* buf,
	size_t count)
{
...
static SIZE_T ps_seq_loop_show(struct device* dev,
	struct device_attribute* attr,
	char* buf)
{
	struct da7280_haptic* haptics = dev_get_drvdata(dev);
 ...
 */
static SIZE_T gpi_seq_id0_store(struct device* dev,
	struct device_attribute* attr,
	const char* buf,
	size_t count)
{
...
static SIZE_T gpi_seq_id0_show(struct device* dev,
	struct device_attribute* attr,
	char* buf)
{
	struct da7280_haptic* haptics = dev_get_drvdata(dev);
...
static SIZE_T gpi_seq_id1_store(struct device* dev,
	struct device_attribute* attr,
	const char* buf,
	size_t count)
{
...
static SIZE_T gpi_seq_id1_show(struct device* dev,
	struct device_attribute* attr,
	char* buf)
{
	struct da7280_haptic* haptics = dev_get_drvdata(dev);
...
static SIZE_T gpi_seq_id2_store(struct device* dev,
	struct device_attribute* attr,
	const char* buf,
	size_t count)
{
...
static SIZE_T gpi_seq_id2_show(struct device* dev,
	struct device_attribute* attr,
	char* buf)
{
	struct da7280_haptic* haptics = dev_get_drvdata(dev);
